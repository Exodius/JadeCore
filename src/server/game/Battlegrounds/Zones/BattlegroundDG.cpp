    ...
    if (GetStatus() == STATUS_IN_PROGRESS)
    {
        ...
        for (int team = 0; team < BG_TEAMS_COUNT; ++team)
        {
            ...
            if (m_lastTick[team] > BG_DG_TickIntervals[points])
            {
                ...
                {
                    //(team == ALLIANCE) ? RewardReputationToTeam(509, 10, ALLIANCE) : RewardReputationToTeam(510, 10, HORDE);
                    m_ReputationScoreTics[team] -= m_ReputationTics;
                }

                if (m_HonorScoreTics[team] >= m_HonorTics)
                {
                    RewardHonorToTeam(GetBonusHonorFromKill(1), (team == ALLIANCE) ? ALLIANCE : HORDE);
        ...
        // Test win condition
        if (m_TeamScores[ALLIANCE] >= BG_DG_MAX_TEAM_SCORE)
            EndBattleground(ALLIANCE);
        if (m_TeamScores[HORDE] >= BG_DG_MAX_TEAM_SCORE)
            EndBattleground(HORDE);

        if (m_flagState[ALLIANCE] == BG_DG_FLAG_STATE_WAIT_RESPAWN)
        {
            m_flagsTimer[ALLIANCE] -= diff;

            if (m_flagsTimer[ALLIANCE] < 0)
            {
                m_flagsTimer[ALLIANCE] = 0;
                RespawnFlag(ALLIANCE, true);
            }
        }
        if (m_flagState[ALLIANCE] == BG_DG_FLAG_STATE_ON_GROUND)
        {
            m_flagsDropTimer[ALLIANCE] -= diff;

            if (m_flagsDropTimer[ALLIANCE] < 0)
            {
                m_flagsDropTimer[ALLIANCE] = 0;
                RespawnFlagAfterDrop(ALLIANCE);
            }
        }
        if (m_flagState[HORDE] == BG_DG_FLAG_STATE_WAIT_RESPAWN)
        {
            m_flagsTimer[HORDE] -= diff;

            if (m_flagsTimer[HORDE] < 0)
            {
                m_flagsTimer[HORDE] = 0;
                RespawnFlag(HORDE, true);
            }
        }
        if (m_flagState[HORDE] == BG_DG_FLAG_STATE_ON_GROUND)
        {
            m_flagsDropTimer[HORDE] -= diff;

            if (m_flagsDropTimer[HORDE] < 0)
            {
                m_flagsDropTimer[HORDE] = 0;
...
bool BattlegroundDG::SetupBattleground()
{
    for (int i = 0; i < BG_DG_DYNAMIC_NODES_COUNT; ++i)
    {
        ...
        {
            TC_LOG_ERROR("sql.sql", "BatteGroundDG: Failed to spawn some object Battleground not created!");
            return false;
        }
    }
...
void BattlegroundDG::Reset()
{
    ...
    m_TeamScores[ALLIANCE]          = 0;
    m_TeamScores[HORDE]             = 0;
    m_lastTick[ALLIANCE]            = 0;
    m_lastTick[HORDE]               = 0;
    m_HonorScoreTics[ALLIANCE]      = 0;
    m_HonorScoreTics[HORDE]         = 0;
    m_ReputationScoreTics[ALLIANCE] = 0;
    m_ReputationScoreTics[HORDE]    = 0;
    m_ResourceInMineCart[ALLIANCE]  = 0;
    m_ResourceInMineCart[HORDE]     = 0;
    ...
    m_TeamScores500Disadvantage[ALLIANCE] = false;
    m_TeamScores500Disadvantage[HORDE]    = false;
    ...
    m_FlagKeepers[ALLIANCE] = 0;
    m_FlagKeepers[HORDE] = 0;

    m_DroppedFlagGUID[ALLIANCE] = 0;
    m_DroppedFlagGUID[HORDE] = 0;

    m_flagState[ALLIANCE] = BG_DG_FLAG_STATE_ON_BASE;
    m_flagState[HORDE] = BG_DG_FLAG_STATE_ON_BASE;
...
void BattlegroundDG::RemovePlayer(Player* player, uint64 guid, uint32 /*team*/)
{
    ...
    if (IsAllianceFlagPickedup() && m_FlagKeepers[ALLIANCE] == guid)
    {
        if (!player)
        {
    ...
    if (IsHordeFlagPickedup() && m_FlagKeepers[HORDE] == guid)
    {
        if (!player)
        {
            TC_LOG_ERROR("bg.battleground", "BattlegroundDG: Removing offline player who has the FLAG!!");
...
void BattlegroundDG::NodeOccupied(uint8 node, Team team)
{
    if (!AddSpiritGuide(node, BG_DG_SpiritGuidePos[node][0], BG_DG_SpiritGuidePos[node][1], BG_DG_SpiritGuidePos[node][2], BG_DG_SpiritGuidePos[node][3], team))
       TC_LOG_INFO("bd.battleground", "Failed to spawn spirit guide! point: %u, team: %u, ", node, team);
...
void BattlegroundDG::EventPlayerCapturedFlag(Player* Source)
{
    ...
    if (Source->GetBGTeam() == ALLIANCE)
    {
        ...
        m_flagState[HORDE] = BG_DG_FLAG_STATE_WAIT_RESPAWN;
    ...
    else
    {
        ...
        m_flagState[ALLIANCE] = BG_DG_FLAG_STATE_WAIT_RESPAWN;
    ...
    uint8 team = Source->GetBGTeam() == ALLIANCE ? ALLIANCE : HORDE;

    m_TeamScores[team] += m_ResourceInMineCart[team == ALLIANCE ? HORDE : ALLIANCE];
    m_ResourceInMineCart[team == ALLIANCE ? HORDE : ALLIANCE] = 0;
...
void BattlegroundDG::EventPlayerDroppedFlag(Player* Source)
{
    if (GetStatus() != STATUS_IN_PROGRESS)
    {
        ...
        if (Source->GetBGTeam() == ALLIANCE)
        {
            ...
            if (GetFlagPickerGUID(HORDE) == Source->GetGUID())
            {
                SetHordeFlagPicker(0);
                Source->RemoveAurasDueToSpell(BG_DG_SPELL_HORDE_FLAG);
            }
        ...
        else
        {
            ...
            if (GetFlagPickerGUID(ALLIANCE) == Source->GetGUID())
            {
                SetAllianceFlagPicker(0);
                Source->RemoveAurasDueToSpell(BG_DG_SPELL_ALLIANCE_FLAG);
            }
    ...
    if (Source->GetBGTeam() == ALLIANCE)
    {
        ...
        if (GetFlagPickerGUID(HORDE) == Source->GetGUID())
        {
            ...
            m_flagState[HORDE] = BG_DG_FLAG_STATE_ON_GROUND;
    ...
    else
    {
        ...
        if (GetFlagPickerGUID(ALLIANCE) == Source->GetGUID())
        {
            ...
            m_flagState[ALLIANCE] = BG_DG_FLAG_STATE_ON_GROUND;
...
void BattlegroundDG::EventPlayerClickedOnFlag(Player* Source, GameObject* target_obj)
{
    ...
    if (target_obj->GetEntry() == BG_OBJECT_A_FLAG_DG_ENTRY || target_obj->GetEntry() == BG_OBJECT_H_FLAG_DG_ENTRY)
    {
            ...
            && BgObjects[BG_DG_OBJECT_A_FLAG] == target_obj->GetGUID())
        {
            ...
            m_flagState[ALLIANCE] = BG_DG_FLAG_STATE_ON_PLAYER;
            ...
            && BgObjects[BG_DG_OBJECT_H_FLAG] == target_obj->GetGUID())
        {
            ...
            m_flagState[HORDE] = BG_DG_FLAG_STATE_ON_PLAYER;
        ...
        uint8 team = GetOtherTeam(Source->GetBGTeam()) == ALLIANCE ? ALLIANCE : HORDE;
...
void BattlegroundDG::HandleAreaTrigger(Player* Source, uint32 Trigger)
{
    ...
    switch (Trigger)
    {
        case 9012:                                          // Alliance Flag spawn
            if (m_flagState[HORDE] && !m_flagState[ALLIANCE])
                if (GetFlagPickerGUID(HORDE) == Source->GetGUID())
                    EventPlayerCapturedFlag(Source);
            break;
        case 9013:                                          // Horde Flag spawn
            if (m_flagState[ALLIANCE] && !m_flagState[HORDE])
                if (GetFlagPickerGUID(ALLIANCE) == Source->GetGUID())
                    EventPlayerCapturedFlag(Source);
            break;
    }
...
{
    builder.AppendState(BG_DG_OP_FLAG_A, ((GetFlagState(ALLIANCE) == BG_DG_FLAG_STATE_ON_PLAYER || GetFlagState(ALLIANCE) == BG_DG_FLAG_STATE_ON_GROUND) ? 2 : 1));
    builder.AppendState(BG_DG_OP_FLAG_H, ((GetFlagState(HORDE)    == BG_DG_FLAG_STATE_ON_PLAYER || GetFlagState(HORDE)    == BG_DG_FLAG_STATE_ON_GROUND) ? 2 : 1));
    ...
    for (uint8 node = 0; node < BG_DG_DYNAMIC_NODES_COUNT; ++node)
    {
        ...
        {
            builder.AppendState(BG_DG_OP_NODEICONS[node][m_Nodes[node]], 2);
            ++ally;
        }
        else if (m_Nodes[node] == BG_DG_NODE_STATUS_HORDE_OCCUPIED)
        {
            builder.AppendState(BG_DG_OP_NODEICONS[node][m_Nodes[node]], 1);
            ++horde;
        }
        else
            builder.AppendState(BG_DG_OP_NODEICONS[node][m_Nodes[node]], 1);;
    ...
    builder.AppendState(BG_DG_OP_OCCUPIED_BASES_ALLY, ally);
    builder.AppendState(BG_DG_OP_OCCUPIED_BASES_ALLY, horde);

    // Team scores
    builder.AppendState(BG_DG_OP_RESOURCES_ALLY, GetTeamScore(TEAM_ALLIANCE));
    builder.AppendState(BG_DG_OP_RESOURCES_HORDE, GetTeamScore(TEAM_HORDE));
}
