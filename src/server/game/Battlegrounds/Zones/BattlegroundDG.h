...
class BattlegroundDG : public Battleground
{
    ...
    private:
        ...
        void SetAllianceFlagPicker(uint64 guid)              { m_FlagKeepers[ALLIANCE] = guid; }
        void SetHordeFlagPicker(uint64 guid)                 { m_FlagKeepers[HORDE] = guid; }
        bool IsAllianceFlagPickedup() const                  { return m_FlagKeepers[ALLIANCE] != 0; }
        bool IsHordeFlagPickedup() const                     { return m_FlagKeepers[HORDE] != 0; }
};
